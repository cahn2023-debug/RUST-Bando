...
export type MapLoadState = 'initializing' | 'ready';
...
export function MapLibreFastRenderer({
    ...
    const [mapLoadState, setMapLoadState] = React.useState<MapLoadState>('initializing');
    ...
    React.useEffect(() => {
        ...
        const initializeMap = () => {
            if (mapInitialized || !map.isStyleLoaded()) return;
            mapInitialized = true;
    ...
    React.useEffect(() => {
        ...
        render();
    }, [contextMap, iconReadyRevision, renderCacheKey, renderCollectionResult, renderFlags, reportCaptureScope, scheduleSelectedFeatureState, selectedFeatureId, setRenderMetrics, updateOpenMetrics]);
    ...
    React.useEffect(() => {
        const map = mapRef.current;
        if (!map || !zoomToTrigger || !map.isStyleLoaded()) return;
    ...
    }, [contextMap, featureDetailsCache, features, mapLoadState, mapState, zoomToTrigger]);
    ...
    return (
        <div
            ...
            data-map-attach-mode="owned"
            data-map-state={mapLoadState}
