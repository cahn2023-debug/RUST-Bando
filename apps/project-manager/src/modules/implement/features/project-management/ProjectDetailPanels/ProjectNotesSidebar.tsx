...
export function ProjectNotesSidebar({ notes, onAdd, onDelete, adding, newTitle, setNewTitle, onSubmit }: any) {
   ...
   useEffect(() => {
      if (adding) newNoteInputRef.current?.focus();
   }, [adding]);
      ...
      <div className="flex flex-col gap-4">
         {adding ? (
            <form onSubmit={onSubmit} className="bg-cad-bg p-3 border border-cad-accent">
               <input ref={newNoteInputRef} value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full bg-cad-surface border border-cad-border p-2 text-[10px] text-white outline-none" placeholder="NOTE TITLE..." />
