...
import { aiApi, projectApi } from './index';
...
describe('typed Tauri API facade', () => {
  ...
  it('keeps project command names and payloads stable', async () => {
    ...
    expect(mockedSafeInvoke).toHaveBeenNthCalledWith(3, 'index_project_files', { projectId: 'project-1' });
  });

  it('routes AI calls through the safe adapter', async () => {
    mockedSafeInvoke.mockResolvedValue({});

    await aiApi.cancelRequest('request-1');

    expect(mockedSafeInvoke).toHaveBeenNthCalledWith(1, 'cancel_ai_request', { requestId: 'request-1' });
  });
});
