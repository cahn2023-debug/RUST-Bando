        ...
        return (
            <div className="flex flex-col items-center justify-center h-full text-cad-text-muted bg-cad-surface p-4 text-center border border-cad-border shadow-2xl font-mono">
                <div className="flex items-center justify-between w-full absolute top-0 p-3 border-b border-cad-border bg-cad-elevated drag-handle cursor-move rounded-t-xl" {...dragHandleProps}>
                    ...
                    <Button variant="ghost" size="sm" icon={X} onClick={onClose} ariaLabel="Đóng" />
                </div>
        ...
        <div className={cn(
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    {(isDirty || isSaving) && <div className="w-1.5 h-1.5 rounded-full bg-cad-accent animate-pulse" />}
                </div>
                <div className="flex items-center gap-1">
                    ...
                    <Button
                        ...
                        size="sm"
                        onClick={onPin}
                        className={cn("uppercase", isPinned && "bg-cad-elevated text-cad-accent")}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black tracking-widest text-cad-text-muted uppercase">
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            icon={expandedSections.specs ? ChevronDown : ChevronRight}
                            onClick={() => toggleSection('specs')}
                            ariaLabel={expandedSections.specs ? 'Thu gọn thông số kỹ thuật' : 'Mở rộng thông số kỹ thuật'}
                        />
                    </div>
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black tracking-widest text-cad-text-muted uppercase">
                        <MapIcon className="w-3 h-3 text-cad-accent" /> Mô phỏng Góc nhìn
                        <div className="flex-1" />
                        <Button
                            variant="ghost"
                            size="sm"
                            icon={expandedSections.simulation ? ChevronDown : ChevronRight}
                            onClick={() => toggleSection('simulation')}
                            ariaLabel={expandedSections.simulation ? 'Thu gọn mô phỏng góc nhìn' : 'Mở rộng mô phỏng góc nhìn'}
                        />
                    </div>
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 text-[10px] font-black tracking-widest text-cad-text-muted uppercase">
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            icon={expandedSections.recognition ? ChevronDown : ChevronRight}
                            onClick={() => toggleSection('recognition')}
                            ariaLabel={expandedSections.recognition ? 'Thu gọn chất lượng nhận diện' : 'Mở rộng chất lượng nhận diện'}
                        />
                    </div>
                        ...
                        <div className="space-y-6 bg-cad-bg p-3 rounded border border-cad-border">
                            ...
                            <div className="space-y-2 pt-2">
                                ...
                                <input
                                    ...
                                    className="w-full h-1.5 bg-cad-elevated rounded-lg appearance-none cursor-pointer accent-purple-400"
                                />
                            </div>
                            ...
                            <div className="space-y-2 pt-4 border-t border-cad-border">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button
                                        ...
                                        className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase transition-all border ${showDORILayers ? 'bg-purple-500/20 border-purple-500/30 text-purple-400' : 'bg-transparent border-cad-border text-cad-text-muted'}`}
                                    >
                                        DORI Map
                                    </button>
                                </div>
                            ...
                            <div className="pt-4 border-t border-cad-border">
                                {lowPowerMode ? (
                                    <div className="flex items-center gap-2 p-2 bg-cad-elevated rounded text-[9px] text-cad-text-muted italic border border-cad-border">
