...
import {
    Plus, Package, Building2, FolderOpen, FileText,
    Clock, Zap, PlayCircle, ExternalLink,
    Cpu, CheckCircle2, User
} from 'lucide-react';
...
export function ContractAnalysisView({
    ...
    useEffect(() => {
        ...
        setData({
            ...
            end_date: initialData.end_date && initialData.end_date !== "---" ? initialData.end_date : calculatedEndDate
        });
    }, [initialData]);
    ...
    const fetchGroups = async () => {
        ...
        } catch (error) { console.error("Fetch groups error:", error); }
    };
    ...
    const handleUpsertGroup = async (group: Partial<ContractExecutionGroup>) => {
        ...
        } catch (error) { console.error("Upsert group error:", error); }
    };
        ...
        return (
            <div className="flex-1 bg-cad-bg overflow-hidden flex flex-col animate-in fade-in duration-500 font-sans">
                <div className="px-8 py-4 border-b border-cad-border bg-cad-bg/80 backdrop-blur-md z-10 flex flex-col gap-2">
                    <h1 className="text-2xl font-black text-white tracking-tighter uppercase leading-none">DETAILED <span className="text-cad-accent">SUMMARY</span></h1>
                ...
                <div className="flex justify-between items-end">
                    ...
                    <div className="flex flex-col items-end gap-2 text-cad-accent">
                        ...
                        <div className="w-64 h-1.5 bg-cad-elevated rounded-full overflow-hidden">
                            <div className="h-full bg-cad-accent rounded-full shadow-[0_0_10px_rgba(34,197,94,0.3)] transition-all duration-1000" style={{ width: '92%' }}></div>
                        </div>
                    </div>
                </div>
                <ProjectInfoBar projectName={projectName} data={data} compact isAnalyzing={isAnalyzing} />
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
                <span className="text-[8px] font-black text-cad-text-muted uppercase tracking-widest">{label}</span>
...
function StatCard({ icon, label, value, subValue, color = "text-white" }: any) {
    return (
        <div className="bg-cad-surface border border-cad-border p-5 rounded-xl hover:border-cad-accent/30 transition-all group">
            ...
            <span className="text-[9px] font-black text-cad-text-muted uppercase tracking-widest mb-1 block">{label}</span>
            <div className={`text-xl font-black ${color} tracking-tight mb-0.5`}>{value}</div>
            <div className="text-[8px] font-bold text-cad-text-secondary uppercase opacity-50">{subValue}</div>
        </div>
    );
}
...
function TaskCard({ status, title, due, color }: any) {
    ...
    return (
        <div className={`p-4 bg-cad-surface border rounded-xl hover:border-cad-accent/30 cursor-pointer transition-all ${colors[color] || colors.slate}`}>
            <div className="flex justify-between items-center mb-2">
                <span className="text-[8px] font-black px-1.5 py-0.5 rounded bg-black/20 uppercase tracking-widest">{status}</span>
