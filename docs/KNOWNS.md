...
- Use `knowns retrieve` when a workflow needs structured context with citations and context-pack assembly.
...
- The file `docs/project_index.html` is the Single Source of Truth (SSOT) for architecture, flows, and troubleshooting.
- **Mandatory Review**: Before implementing any new feature or modifying core logic, agents MUST read `docs/project_index.html`.
- **Mandatory Update**: After completing any task that changes the project structure, adds commands, or resolves a significant bug, agents MUST update `docs/project_index.html` with the new information.
- Failure to maintain this file is considered a critical technical debt.
