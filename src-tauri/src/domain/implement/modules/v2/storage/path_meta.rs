...
pub fn validate_metadata(meta: &serde_json::Value) -> Result<(), String> {
    if let Some(obj) = meta.as_object() {
        for key in obj.keys() {
            let valid = key == "system" || key == "analysis" || key == "custom" || key.starts_with("plugin_");
            if !valid {
                return Err(format!("[V2 META] Invalid namespace: '{}'. Must be system|analysis|custom|plugin_*", key));
            }
        }
    }
    Ok(())
